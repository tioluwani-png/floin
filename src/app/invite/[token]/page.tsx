...
export default function InvitePage() {
  ...
  // Load invite details — re-runs when auth state changes
  useEffect(() => {
    if (!supabase) {
      setStatus('error')
      setErrorMessage('App not configured. Please try again later.')
      return
    }

    // If user is not signed in, show sign-in prompt immediately
    if (!isSignedIn) {
      setStatus('needs-auth')
      return
    }

    // User is signed in — verify Supabase session and load invite
    let cancelled = false
    ...
    async function loadInvite() {
      ...
      try {
        // Ensure Supabase has an active session
        ...
        const { data: bizData } = await supabase!
          ...
          .single()
        const biz = bizData as { name: string; user_id: string } | null

        if (cancelled) return

        setInvite({ ...inv, businessName: biz?.name || 'a business' })
        ...
        try {
          ...
          if (members.some(m => m.user_id === user!.id)) {
            if (!cancelled) setStatus('already-member')
            return
          }
        } catch {
          // If we can't read members (RLS), that's fine — we're not a member
        }
        ...
        <div className="mt-8 rounded-2xl bg-white p-6 shadow-sm border border-border/50">
          ...
          {status === 'needs-auth' && (
            <div className="text-center">
              ...
              <h2 className="mt-3 text-lg font-bold">You&apos;ve been invited</h2>
              <p className="mt-1.5 text-sm text-muted">
                Sign in with Google to join this business on Floin.
              </p>
              <button
          ...
          {status === 'error' && (
            <div className="text-center">
              ...
              <h2 className="mt-3 text-lg font-bold">Something went wrong</h2>
